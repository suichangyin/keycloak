...
import org.keycloak.providers.phone.authentication.requiredactions.UpdatePhoneNumberRequiredAction;
...
import org.keycloak.providers.phone.providers.jpa.TokenCode;
...
public class DefaultPhoneVerificationCodeProvider implements PhoneVerificationCodeProvider {
    ...
    public void cleanUpAction(UserModel user) {
        ...
        if (socp.isConfiguredFor(getRealm(), user, PhoneOtpCredentialModel.TYPE)) {
            CredentialModel credential = user.credentialManager().getStoredCredentialsByTypeStream(PhoneOtpCredentialModel.TYPE).findFirst().get();
